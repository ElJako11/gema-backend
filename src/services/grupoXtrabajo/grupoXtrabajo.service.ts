...
export const asignarGrupo = async (
  params: CreateGrupoXTrabajoParams,
  tx?: Tx
) => {
...
export const desasignarGrupo = async (idT: number, idG: number) => {
  const deleted = await db
    .delete(grupoXtrabajo)
    .where(and(eq(grupoXtrabajo.idT, idT), eq(grupoXtrabajo.idG, idG)))
    .returning();
...
export const getGruposAsignadosATrabajo = async (idTrabajo: number) => {
  try {
    const grupos = await db
      ...
      .from(grupoDeTrabajo)
      .innerJoin(grupoXtrabajo, eq(grupoDeTrabajo.id, grupoXtrabajo.idG))
      .innerJoin(trabajo, eq(trabajo.idTrabajo, grupoXtrabajo.idT))
